...
class ExpirationDateScope implements Scope {
    ...
    {
        $column = $model->getQualifiedExpirationDateColumn();

        $query = $builder->getQuery();

        $builder->where(function($query) use ($column, $model) {
            $query->where($column, '>', $model->getExpirationDateValue());
        ...
        $this->addWithExpirationDate($builder);
    ...
    public function remove(Builder $builder, Model $model)
    {
        ...
        {
            if ($this->isExpirationDateConstraint($where, $column, $model->getExpirationDateValue()))
            {
     ...
     * @return boolean
     */
    protected function isExpirationDateConstraint(array $where, $column, $value)
    {
     ...
     * @param \Illuminate\Database\Eloquent\Builder  $builder
     */
    protected function addWithExpirationDate(Builder $builder)
    {
        $builder->macro('withExpirationDate', function(Builder $builder)
        {
