<?php

namespace Snap\Admin\Modules;

use App\Models\User;
use Illuminate\Http\Request;
use Snap\Admin\Http\Controllers\ResourceModuleController;
use Snap\Admin\Models\Role;
use Snap\Form\Processors\RelationshipToManyProcessor;

class RoleModule extends ResourceModule
{
    use \Snap\Admin\Modules\Traits\FormTrait;
    use \Snap\Admin\Modules\Traits\TableTrait;
    use \Snap\Admin\Modules\Traits\NavigableTrait;
    use \Snap\Admin\Modules\Traits\DeletableTrait;
    use \Snap\Admin\Modules\Traits\BasePermissionsSelectionTrait;

    protected $parent = 'user';
    protected $handle = 'role';
    protected $name = 'Role';
    protected $pluralName = 'Roles';
    protected $menuParent = 'user';
    protected $menuLabel = 'Roles';
    protected $description = '';
    protected $version = '1.0.0';
    protected $icon = '';
    protected $path = __DIR__;
    protected $modules = [];
    protected $uri = null;
    protected $permissions = [];
    protected $config = null;
    protected $routes = [];
    protected $controller = ResourceModuleController::class;
    protected $model = Role::class;
    protected $scopes = ['active', 'inActive', 'onlyTrashed'];

    public function query($query) {
        $query->where('name', '!=', 'super-admin');
    }

    public function table($table, Request $request)
    {
        $table
            ->format('\Snap\Admin\Ui\Formatters\EditInline::format', ['name', 'guard_name'])
        ;
    }

    public function form($form, Request $request)
    {
        $form->scaffold([
            'props' => ['users' => ['type' => 'multiselect', 'options' => User::whereHas('roles', function($q)  {
                $q->whereNotIn('id', User::role('super-admin')->pluck('id'));
            })->lists('name'), 'post_process' => RelationshipToManyProcessor::class]],
            'except' => ['guard_name']]
        );

        $form->addMulticheckbox('permissions', ['group' => 'Permissions', 'label' => false, 'options' => $this->getPermissionOptions(), 'post_process' => function($value, $input, $request, $resource){
            $this->processBelongToMany('permissions', $request, $resource);
        }])->move('permissions', 'after:users');
    }


}