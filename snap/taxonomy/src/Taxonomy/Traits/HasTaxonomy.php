...
namespace Snap\Taxonomy\Traits;

use Snap\Taxonomy\Models\Taxonomy;

trait HasTaxonomy {

    public function hasTaxonomy($vocabulary = null)
    {
        //$belongsTo = $this->sharedRelationship(Taxonomy::class, $vocabulary ['method' => debug_backtrace()[1]['function']]);
        $shared = $this->sharedRelationship(Taxonomy::class);

        if (!empty($vocabulary)) {

            if (is_int($vocabulary)) {
                $shared->where('vocabulary_id', $vocabulary);

            } elseif (is_string($vocabulary)) {
                $shared->whereHas('vocabulary', function($query) use ($vocabulary) {
                    $query->where('handle', $vocabulary)->orWhere('name', $vocabulary);
                });
            }
        }
